...
# Dev-only Inworld WebRTC smoke-test endpoints

#

# These exist ONLY to answer "can Inworld produce audible audio through its

# own WebRTC path?" — completely independent of the LiveKit / WebSocket bridge

# which still has the no-``response.output_audio.delta`` problem.

#

# The browser never sees ``INWORLD_API_KEY``: the dev page calls these BFF

# routes, which forward the SDP offer / ICE fetch to Inworld on the server.

# The LiveKit / WebSocket bridge code paths are deliberately untouched so a

# rollback remains one revert away.

# ---------------------------------------------------------------------------


import aiohttp  # already imported at top of file; re-export is fine



def _inworld_bearer_token() -> str:
...
def _inworld_web_auth_header() -> str | None:

    """Authorization header for the Inworld WebRTC REST endpoints
...
@app.get("/api/inworld/webrtc/ice")

async def inworld_webrtc_ice() -> JSONResponse:

    """Forward ICE-server lookup to Inworld. Returns whatever JSON Inworld sends.
    ...
    if not api_key:
        ...
        return JSONResponse(
            ...
            {"error": "inworld_webrtc_ice_unavailable reason=INWORLD_API_KEY_missing"},
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=15)) as sess:
            ...
            async with sess.get(
                ...
                headers={"Authorization": _inworld_web_auth_header()},
                ...
                logger.info(
                    ...
                    "inworld_webrtc_ice_proxy status=%s bytes=%s",
                ...
                if r.status == 200:
                    ...
                    try:
                        ...
                        return JSONResponse(
                            ...
                            {"raw_body": body[:4096]}, status_code=200,

                        )

                return JSONResponse(

                    {"error": "inworld_webrtc_ice_failed", "status": r.status, "body": body[:1024]},
        ...
        logger.error("inworld_webrtc_ice_proxy_exception error=%s", exc)

        return JSONResponse(

            {"error": "inworld_webrtc_ice_exception", "detail": str(exc)},
