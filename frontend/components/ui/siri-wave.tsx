...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    col *= res;
    // Alpha follows brightness so quiet areas are transparent, not a black box.
...
export function SiriWave({
  ...
  React.useEffect(() => {
    ...
    if (!canvas) return
    // premultipliedAlpha: false so the shader's own alpha (brightness-driven,
    // not a flat 1.0) composites correctly against whatever sits behind the
    // canvas instead of the browser darkening it toward black.
