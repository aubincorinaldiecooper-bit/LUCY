...
def _inject_response_mode_note(turn_ctx: object, turn_kind: str, detected_intent: str) -> bool:
    """VITA-style response-mode control: action/tool turns get a focused-execution
    note so they read differently from companion conversation. Conversation turns
    are untouched."""
    ...
    note = (
        ...
        "Perform the requested action directly: call the needed tool first if one applies, "
        "answer briefly and concretely, and skip companion small talk for this turn."
    )
    add_message = getattr(turn_ctx, "add_message", None)
    if not callable(add_message):
...
class LucyAgent(Agent):
    ...
    async def on_user_turn_completed(self, turn_ctx, new_message) -> None:
        ...
        if turn_policy.decision == "HOLD_FOR_CONTINUATION":
            ...
            # Responsive hold (full-duplex): wait for the reply deadline in small
            # steps and yield immediately when the user continues. The held
            # fragment stays stored so the continuation turn can merge with it;
            # this turn produces no response instead of generating a stale one
            # after the deadline.
